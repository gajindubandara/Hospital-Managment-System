...
if(!empty($_POST["option"])) {
    ...
    if ($_POST["option"]=="All"){
        ...
        $query = "SELECT `name`,`sField`, `qual`, `rps`, `ppd`, `state` FROM `doctor` WHERE`sField` ='" . $_POST["option"] . "' && `state`='active'";
        ...
        $arr = array();
         ...
         if($count>0) {
            foreach ($result as $row) {
                $objArray = array('name'=>$row['name'],'field'=>$row['sField'],'qual'=>$row['qual'],'rps'=>$row['rps'],'ppd'=>$row['ppd']);
